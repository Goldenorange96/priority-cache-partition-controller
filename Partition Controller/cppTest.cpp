...
void doPartition (int amount, int size, INFO pids[])
{
	...
	if(amount > 0)
	{
		...
		{	
			tmpStr = "";
			calcBinRange(size, pids[i].part-1, tmpStr, amount-1);
			tmpStr = generatePartitionsCommand(tmpStr, pids[i].part, pids[i].pid);
			system(tmpStr.c_str());
...
void repartition (int amount, INFO * pids, int totalCM)
{
	...
	for (int i = 0; i < amount; i++)
	{
		...
		execute = generatePartitionsCommand(range, pids[i].part, pids[i].pid);
...
{	
	...
	while(1)
	{
		...
		cachemiss = 0;
		//this is used for given same pid to same partition by lower pid gets lower partition number
		if(newprio[0].pid < newprio[1].pid )
		{
			newprio[0].part = 1;
			newprio[1].part = 2;
		}
		...
		if((cachemiss = calculateTotalCM(prio, newprio)) < 1)
		{
		...
		{
			repartition(prio, newprio, cachemiss);
			
		}
		
		assignoprio(oPrio, newOprio);

		cout << "Cache misses: "<< cachemiss << "prio: " << prio << "oprio: "<< oPrio << endl << endl;
	...
	//doPartition (prio, partSize, newprio);
	//g++ -std=c++11 -o cppTest cppTest.cpp -pthread /home/jj/Downloads/papi/src/libpapi.a
		//int tt [3] = {100, 50, 70};
